...
async function main() {
  ...
  const identity1 = _entity.Entity.from_hex('6e8339a0c6d51fc58b4365bf2ce18ff2698d2b8c40bb13fcef7e1ba05df18e4b');

  const identity2 = _entity.Entity.from_hex('e833c747ee0aeae29e6823e7c825d3001638bc30ffe50363f8adf2693c3286f8');

  const tx = await api.tokens.transfer(identity1, identity2, 2500, 20); // We Verify that the transaction is the submitted transaction is the sent transaction
  // TxContents object (below contents variable) contains all properties sent to ledger in transaction API call

  const contents = await api.tx.contents(tx); //below we access a subset of the properties of our TxContents object

  const valid_until = contents.valid_until;
  const valid_from = contents.valid_from;
  const from_address = contents.from_address;
  const transfers = contents.transfers; // iterate over the transfers in the transaction, which is singular in this instance
  // ${amount.toNumber()}
  ...
  const unused_entity = new _entity.Entity(); // Check the amount being transferred to a particular address; zero in the below instance
