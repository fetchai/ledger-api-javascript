...
async function main() {
  let balance, balance1, balance2, signatories, txs, tx; // create the APIs
  ...
  const other_identity = _entity.Entity.from_hex('e833c747ee0aeae29e6823e7c825d3001638bc30ffe50363f8adf2693c3286f8');

  balance = await api.tokens.balance(multi_sig_identity);
  console.log('\nOriginal balance of multi_sig_identity: ', balance.toString()); // Transfers can happen normally without a deed
  ...
  console.log('\nBalance 2:', balance2.toString()); // Submit deed

  console.log('\nCreating deed...');
  const deed = new _deed.Deed();
  board.forEach(item => {
  ...
  await api.sync([tx], 20).catch(errors => sync_error(errors)); // Original address can no longer validate transfers
  ...
  console.log('\nBalance 1:', balance1.toString());
  balance2 = await api.tokens.balance(other_identity);
  console.log('\nBalance 2:', balance2.toString()); // Some entities may have more voting power
  ...
  console.log('\nBalance 2:', balance.toString()); // Amend the deed
  ...
  await api.sync([txs]).catch(errors => sync_error(errors)); // Single member no longer has enough voting power
  ...
  console.log('\nBalance 1:', balance1.toString());
  balance2 = await api.tokens.balance(other_identity);
  console.log('\nBalance 2:', balance2.toString()); // Warning: if no amend threshold is set, future amendments are impossible
  ...
  await api.sync([tx]).catch(errors => console.log(`\nFurther amendment failed as expected. \n\nTransaction status: ${errors[0].get_status()}\n`));
}
