...
var _contracts = require("../../fetchai/ledger/api/contracts");

var _deed = require("../../fetchai/ledger/crypto/deed");

var _ledger = require("../../fetchai/ledger");

var _crypto = require("../../fetchai/ledger/crypto");

var _contracts2 = require("../../contracts");
...
async function print_address_balances(api, contract, addresses) {
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    query = await contract.query(api, 'balanceOf', {
      address: addresses[i]
    });
...
async function main() {
  let txs, tx, nonce; // We generate an identity from a known key, which contains funds.

  const multi_sig_identity = _entity.Entity.from_hex('e833c747ee0aeae29e6823e7c825d3001638bc30ffe50363f8adf2693c3286f8'); // create our first private key pair


  const multi_sig_address = new _crypto.Address(multi_sig_identity); // generate a board to control multi-sig account, with variable voting weights

  const board = [];
  board.push({
    member: _entity.Entity.from_hex('6e8339a0c6d51fc58b4365bf2ce18ff2698d2b8c40bb13fcef7e1ba05df18e4b'),
    voting_weight: 1
  });
  board.push({
  ...
  board.push({
    member: _entity.Entity.from_hex('7da0e3fa62a916238decd4f54d43301c809595d66dd469f82f29e076752b155c'),
    voting_weight: 1
  });
  board.push({
  ...
  const address2 = new _crypto.Address(entity2); // build the ledger API

  const api = new _api.LedgerApi('127.0.0.1', 8000); // create contract factory

  const contract_factory = new _contracts.ContractTxFactory(api); // create a multisig deed for multi_sig_identity

  console.log('\nCreating deed...');
  const deed = new _deed.Deed(multi_sig_identity);
  board.forEach(item => deed.set_signee(item.member, item.voting_weight));
  deed.set_amend_threshold(4); // Both the transfer and execute thresholds must be met to create a contract
  ...
  deed.set_threshold('TRANSFER', 2); //Submit deed

  txs = await api.tokens.deed(multi_sig_identity, deed).catch(error => console.log(error));
  await api.sync([txs]).catch(errors => sync_error(errors)); // create the smart contract

  console.log('\nSetting up smart contract\n'); // A nonce is an optional attribute for a contract.

  nonce = Buffer.from('590953aea8a09c51', 'hex');
  const contract = new _ledger.Contract(_contracts2.MUTLISIG_CONTRACT, multi_sig_identity, nonce);
  ...
  await api.sync([txs]).catch(errors => sync_error(errors)); // print the current status of all the tokens

  console.log('\n-- BEFORE --\n');
  await print_address_balances(api, contract, [multi_sig_address, address2]); // transfer from one to the other using our newly deployed contract
