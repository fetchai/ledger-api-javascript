...
async function main() {
    let balance, balance1, balance2, txs, tx;
    // create the APIs
    ...
    tx = await TokenTxFactory.transfer(multi_sig_identity, other_identity, 250, 20, board.map(item => item.member))
   ...
   const signed_txs = []
    ...
    for (const board_member of board) {
        ...
        let signer_tx = await TokenTxFactory.transfer(multi_sig_identity, other_identity, 250, 20, board.map(item => item.member))
