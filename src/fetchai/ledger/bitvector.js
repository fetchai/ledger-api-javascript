...
export class BitVector {
    ...
    instance_bytes() {
        if (this._buffer) {
            // TODO: Improve logic
            return new Buffer(this._buffer.toString('hex').match(/.{2}/g).reverse().join(""), 'hex')
        }
        return ''
    }
    ...
    from_bytes(data, bit_size) {
        ...
        // TODO: Improve logic
        bits._buffer = new Buffer(data.toString('hex').match(/.{2}/g).reverse().join(""), 'hex')
        return bits
    }
    ...
    as_binary() {
        ...
        for (let n of data) {
            // TODO: Improve logic
            output += Array.from(Array(8).keys()).reverse().map(value => String(1 & (Number(n) >> value))).join('')
        }
        return output
    }
